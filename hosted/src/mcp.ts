...
import { commitIntakeBatch, getIntakeContext, searchLibrary, type IntakeBatchResult } from "./intake.ts";
...
export async function handleLibraryMcp(
  db: D1Database,
  ...
  try {
    return { status: 200, body: { jsonrpc: "2.0", id, result: await dispatch(db, capability, req.method, req.params) } };
  } catch (error) {
...
async function dispatch(db: D1Database, capability: LibraryCapability, method: string, params: unknown): Promise<unknown> {
...
async function callTool(db: D1Database, capability: LibraryCapability, params: unknown): Promise<unknown> {
  ...
  try {
    if (name === "get_library_intake_context") {
      ...
      return toolResult({ ok: true, capabilityVersion: 1, ...(await getIntakeContext(db, capability.libraryId)) });
    }
    if (name === "search_library") {
      return toolResult({ ok: true, items: (await searchLibrary(db, capability.libraryId, args ?? {})).items });
    }
    return toolResult(boundCreate(await commitIntakeBatch(db, capability.libraryId, "agent", args ?? {})));
  } catch (error) {
